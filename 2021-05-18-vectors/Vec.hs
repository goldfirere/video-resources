...
plusComm (SSucc (m' :: SNat m')) n = case mPlusSucc @m' n of Refl -> case plusComm m' n of Refl -> Refl

type PlusVec :: Nat -> Type -> Type
data PlusVec n a where
  MkPlusVec :: Vec m a -> Vec p a -> PlusVec (m + p) a

span :: (a -> Bool) -> Vec n a -> PlusVec n a
span _ Nil = MkPlusVec Nil Nil
span p (x :> xs) | p x = case span p xs of MkPlusVec trues others -> MkPlusVec (x :> trues) others
